...
def configure(logging_file, log_level=logging.INFO, prefix='', write_to_stdout=True):
    ...
    formatter = MultiLineFormatter("{}%(asctime)s [%(levelname)-5s]  %(message)s".format(prefix),
                                   "%Y-%m-%d %H:%M:%S")
